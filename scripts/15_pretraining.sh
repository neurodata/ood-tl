...
# python3 train_singlehead.py -m seed=10 reps=10 deploy=True device=cuda:2 tag=15_pretraining/wrn10_2/aug ptw=False task.dataset=split_cifar10 net=wrn10_2 task.custom_sampler=False task.target=1 task.ood=[4] task.m_n=0,1,2,3,4,5,10,20 task.augment=True hp.bs=128 hp.epochs=100 hp.lr=0.01

#  cifar10 Ti vs Tj (with augmentation + pretraining)
python3 train_singlehead.py -m seed=10 reps=10 deploy=True device=cuda:1 tag=15_pretraining/wrn10_2/aug_ptw ptw=True task.dataset=split_cifar10 net=wrn10_2 task.custom_sampler=False task.target=1 task.ood=[4] task.m_n=0,1,2,3,4,5,10,20 task.augment=True hp.bs=128 hp.epochs=100 hp.lr=0.001
