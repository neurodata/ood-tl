...
def evaluate(cfg, net, testloader, run_num, silent=False):
    device = torch.device(cfg.device if torch.cuda.is_available() else 'cpu')

    if silent and device != 'cpu':
        device = 'cuda'
...
def train_tune(tune_config, cfg, net, dataHandler):
    # tune_config
    #   lr:
    #   wd:
    #   bs:
    tune_config['bs'] = 2 ** (int(tune_config['bs']))
    ...
    trainloader = dataHandler.get_data_loader(
        train=True, batch=tune_config['bs'])
    testloader = dataHandler.get_data_loader(
        train=False, batch=tune_config['bs'])
    ...
    for epoch in range(cfg.hp.epochs):
        ...
        err = evaluate(cfg, net, testloader, 0, True)

        tune.report(accuracy=1 - err)
