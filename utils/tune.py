...
def search_alpha(net, dataset, n, hp, gpu, sensitivity=0.05, is_task_id=True, val_split=0.1, SEED=1996):
    def wif(id):
        """
    ...
    # alpha_range = np.concatenate((np.arange(0.5, 0.9, 0.05), np.arange(0.9, 1+1e-4, 0.001)))
    ...
    for alpha in alpha_range:
        ...
        for rep in range(1):
            ...
            optimizer = torch.optim.SGD(tune_net.parameters(), 
                                        lr=hp['lr'],
                                        momentum=0.9, 
                                        nesterov=True,
